...
export async function criarPropostaCasa(dados: PropostaCasaDTO) {
    ...
    return {
        message: "Proposta casa criado com sucesso",
...
export async function atualizarPropostaCasa(id: string, dados: Partial<PropostaCasaDTO>) {
    ...
    const propostaCasaAtualizado = await prisma.propostaCasa.update({
        where: { id_proposta_casa: id },
        data: {
            id_proposta_casa: dados.id_proposta_casa ?? propostaCasaExistente.id_proposta_casa,
            ...
            valor_ofertado: dados.valor_ofertado ?? propostaCasaExistente.valor_ofertado,
...
export async function deletarPropostaCasa(id: string) {
    ...
    if(!propostaCasa) {
        throw new Error("Proposta casa não econtrado");
    }
    ...
    return {
        message: "Proposta casa deletado",
    };
}