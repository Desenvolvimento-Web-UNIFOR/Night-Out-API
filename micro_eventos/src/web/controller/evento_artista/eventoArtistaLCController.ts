...
export const criarEventoArtista: RequestHandler = async (req, res) => {
    try {
        ...
        const novoEventoArtista = await eventoArtistaService.criarEventoArtista(dados);
...
export const buscarEventoArtistaProId: RequestHandler = async (req, res) => {
    try {
        const {id} = req.params;
        const eventoArtista = await eventoArtistaService.buscarEventoArtistaPorId(id);
...
export const editarEventoArtista: RequestHandler = async (req, res) => {
    try {
        ...
        const eventoArtistaAtualizado = await eventoArtistaService.atualizarEventoArtista(id, dados);
